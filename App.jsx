...
function HistoryCard({ snap }) {
  ...
  Object.entries(snap.sections || {}).forEach(([sectionName, section]) => {
    section.items.forEach(item => {
      ...
      if (st === "low" || st === "zero") lows.push({ sectionName, item, value, st });
    });
  });
    ...
    <div className="card history-card">
      <button className="history-head" onClick={() => setOpen(!open)}>
        <div>
          ...
          {snap.sentByEmployee && <span className="tag blue">أُرسل للمدير</span>}
          <p>{lows.length ? `${lows.length} صنف ناقص` : "لا توجد نواقص"}</p>
        </div>
        <span>{open ? "⌃" : "⌄"}</span>
      </button>
      ...
      {open && (
        <div className="history-body">
          {lows.length === 0 ? (
            <p className="muted center">الجرد ممتاز ولا توجد نواقص.</p>
          ) : lows.map(({ sectionName, item, value, st }) => (
            <div className="row small" key={`${sectionName}-${item.id}`}>
              <span>{sectionName} — {item.name}</span>
              <strong className={st}>{value} {item.unit}</strong>
            </div>
          ))}
        </div>
      )}
...
export default function App() {
  ...
  async function loadHistory() {
    try {
      ...
      setHistory(snap.docs.map(doc => ({ firestoreId: doc.id, ...doc.data() })));
    } catch (error) {
      console.error(error);
    ...
    Object.entries(sections).forEach(([sectionName, section]) => {
      section.items.forEach(item => {
        list.push({
          ...
          section,
          item,
          value: quantities[sectionName]?.[item.id] ?? ""
        });
      });
    });
    return list;
  ...
  const lowList = allItems.filter(({ item, value }) => {
    const st = getStatus(value, item.minQty);
    return st === "low" || st === "zero";
  });
  ...
  async function saveSnap(sentByEmployee = false) {
    setLoading(true);
    try {
      const snap = {
        id: Date.now(),
...
${items.map(item => `• ${item.name}: ${orderQty[item.id]} ${item.unit}`).join("\n")}`;
  ...
  function shareWhatsapp(sectionName) {
    window.open("https://wa.me/?text=" + encodeURIComponent(orderText(sectionName)), "_blank");
  }

  async function copyOrder(sectionName) {
    try {
      await navigator.clipboard.writeText(orderText(sectionName));
      showToast("تم نسخ الطلب");
    } catch {
      showToast("انسخ الطلب يدويًا");
    }
  }
  ...
  return (
    <main className="app">
      <header className="top">
        <div>
          <p>فرع أبو زهرة — {todayStr()}</p>
          <h1>🍒 كرز وعنب</h1>
        </div>
        <div className="counter">
          <strong>{enteredCount}/{allItems.length}</strong>
          <span>تم إدخاله</span>
        </div>
      </header>

      <div className="progress">
        <span style={{ width: `${allItems.length ? (enteredCount / allItems.length) * 100 : 0}%` }} />
      </div>
      ...
      <section className="content">
        {activeTab === "جرد" && (
          <>
            <div className="note">
              أدخل العدد الفعلي لكل صنف. الأصناف الأقل من الحد الأدنى تظهر تلقائيًا في تبويب النواقص.
            </div>

            {Object.entries(sections).map(([sectionName, section]) => {
              const isOpen = openSection === sectionName;
              const entered = section.items.filter(item => quantities[sectionName]?.[item.id] !== "").length;
              const lows = section.items.filter(item => {
                const st = getStatus(quantities[sectionName]?.[item.id], item.minQty);
                return st === "low" || st === "zero";
              }).length;
              ...
              return (
                <div className="section-card" key={sectionName}>
                  ...
                  >
                    ...
                    <span>
                      <strong style={{ color: section.color }}>{sectionName}</strong>
                      <small>{entered}/{section.items.length} صنف {lows > 0 ? `— ${lows} ناقص` : ""}</small>
                    </span>
                    <b>{isOpen ? "⌃" : "⌄"}</b>
                  </button>
                  ...
                  {isOpen && (
                    <div className="items">
                      {section.items.map(item => {
                        const value = quantities[sectionName]?.[item.id] ?? "";
                        const st = getStatus(value, item.minQty);
                        return (
                          <div className={`item ${st}`} key={item.id}>
                            <div>
                              <strong>{item.name}</strong>
                              <small>{item.unit} — حد أدنى {item.minQty}</small>
                            </div>
                            <input
                              inputMode="decimal"
                              type="number"
                              value={value}
                              placeholder="العدد"
                              onChange={e => setQty(sectionName, item.id, e.target.value)}
                            />
                          </div>
                        );
                      })}
        ...
        {activeTab === "نواقص" && (
          <>
            <div className="note blue">
              للمدير: راجع النواقص، اكتب كمية الطلب، ثم أرسل طلب المورد عبر واتساب.
            </div>

            {Object.entries(sections).map(([sectionName, section]) => {
              const sectionLows = section.items.filter(item => {
                const st = getStatus(quantities[sectionName]?.[item.id], item.minQty);
                return st === "low" || st === "zero";
              });
                ...
                <div className="card" key={sectionName}>
                  <div className="supplier-head">
                    <div>
                      ...
                      <p>{sectionLows.length ? `${sectionLows.length} ناقص` : "كل الأصناف جيدة"}</p>
                    </div>
                    <button onClick={() => setOrderOpen(sectionName)}>طلب المورد</button>
                  </div>
                  ...
                  {section.items.map(item => {
                    ...
                    return (
                      <div className={`row ${st}`} key={item.id}>
                        <span>{item.name}</span>
                        <strong>{value === "" ? "—" : value} {item.unit}</strong>
                        <small>{statusText[st]}</small>
                      </div>
                    );
                  })}
                </div>
        ...
        {activeTab === "السجل" && (
          <>
            {history.length === 0 ? (
              <div className="empty">لا توجد جرود محفوظة بعد</div>
            ) : (
              history.map(snap => <HistoryCard key={snap.firestoreId || snap.id} snap={snap} />)
            )}
          </>
        )}
      </section>
      ...
      {activeTab === "جرد" && (
        <footer className="bottom">
          <button className="secondary" disabled={loading} onClick={() => saveSnap(false)}>
            💾 حفظ
          </button>
          <button className="primary" disabled={loading} onClick={() => setSendOpen(true)}>
            📤 إرسال للمدير
          </button>
        </footer>
      )}
      ...
      {sendOpen && (
        <Modal title="إرسال الجرد للمدير" onClose={() => setSendOpen(false)}>
          <div className="modal-content">
            <p>سيتم حفظ الجرد في Firebase، ثم تصفير شاشة الجرد لجرد جديد.</p>
            <div className="summary">
              <span>إجمالي الأصناف</span><strong>{allItems.length}</strong>
              <span>تم إدخالها</span><strong>{enteredCount}</strong>
              <span>الأصناف الناقصة</span><strong>{lowList.length}</strong>
            </div>
            <button className="primary wide" disabled={loading} onClick={sendToManager}>
              تأكيد الإرسال
            </button>
          </div>
        </Modal>
      ...
      {orderOpen && (
        <Modal title={`طلب المورد — ${orderOpen}`} onClose={() => setOrderOpen(null)}>
          <div className="modal-content">
            {sections[orderOpen].items.map(item => {
              const value = quantities[orderOpen]?.[item.id] ?? "";
              return (
                <div className="order-row" key={item.id}>
                  <div>
                    <strong>{item.name}</strong>
                    <small>الرصيد: {value === "" ? "—" : value} {item.unit}</small>
                  </div>
                  <input
                    type="number"
                    inputMode="decimal"
            ...
            <div className="actions">
              <button className="secondary" onClick={() => copyOrder(orderOpen)}>نسخ</button>
              <button className="whatsapp" onClick={() => shareWhatsapp(orderOpen)}>واتساب</button>
            </div>
          </div>
        </Modal>
      )}
