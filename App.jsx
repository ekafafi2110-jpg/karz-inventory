...
function SnapshotCard({snap}){
  const [open,setOpen]=useState(false); if(!validSnapshot(snap))return null;
  return <div className="card history-card"><button className="history-head" onClick={()=>setOpen(!open)}><div><strong>{snap.date}</strong><span className={snap.status==="submitted"?"tag blue":"tag"}>{snap.status==="submitted"?"مرسل للمدير":"مسودة محفوظة"}</span><p>{snap.branch||"فرع أبو زهرة"} — {countLow(snap)} صنف ناقص</p></div><span>{open?"⌃":"⌄"}</span></button>{open&&<div className="history-body">{Object.entries(snap.sections).map(([sk,sec])=><div key={sk}><h4>{sec.icon} {sk}</h4>{sec.items.map(it=>{ const v=snap.quantities?.[sk]?.[it.id]; const st=getStatus(v,it.minQty); return <div className="row small" key={it.id}><span>{it.name}</span><strong className={st}>{v===""||v===undefined?"—":v} {it.unit}</strong></div>; })}</div>)}</div>}</div>;
}

function ManagerSnapshot({snap}){
  const [open,setOpen]=useState(false); const [openSec,setOpenSec]=useState(null); const [order,setOrder]=useState({}); if(!validSnapshot(snap))return null;
...
export default function App(){
  ...
  const submitted=history.filter(s=>s.status==="submitted"||s.sentByEmployee);
  const tabs=[{key:"جرد",label:"جرد"},{key:"نواقص",label:"نواقص",badge:submitted.length},{key:"السجل",label:"السجل",badge:history.length},{key:"إعدادات",label:"إعدادات"}];
  ...
  async function saveSnap(status="draft"){
    setLoading(true); try{ const snap={id:Date.now(),date:todayStr(),status,sentByEmployee:status==="submitted",branch:"فرع أبو زهرة",sections:JSON.parse(JSON.stringify(sections)),quantities:JSON.parse(JSON.stringify(quantities)),createdAt:serverTimestamp()}; await addDoc(collection(db,FIREBASE_COLLECTION),snap); setHistory(p=>[{...snap,createdAt:new Date().toISOString()},...p]); if(status==="draft"){ localStorage.setItem(DRAFT_KEY,JSON.stringify(snap)); showToast("تم حفظ الجرد كمسودة"); } else { resetInventory(); showToast("تم إرسال الجرد للمدير وتصفير الشاشة"); } }catch(e){ console.error(e); showToast("حدث خطأ أثناء الحفظ"); }finally{ setLoading(false); setSendOpen(false); }
  }
  ...
  return <main className="app">
    ...
    {sendOpen&&<Modal title="إرسال الجرد للمدير" onClose={()=>setSendOpen(false)}><div className="modal-content"><p>سيتم حفظ لقطة كاملة من شاشة الجرد، وإظهارها في شاشة النواقص/المدير، ثم تصفير شاشة الموظف.</p><button className="primary wide" disabled={loading} onClick={()=>saveSnap("submitted")}>تأكيد الإرسال</button></div></Modal>}
