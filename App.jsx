...
function ManagerSnapshot({snap,onDone}){
  ...
  async function closeReview(type){ if(!snap.firestoreId){alert("لا يوجد معرف للحفظ"); return;} setBusy(true); try{ await updateDoc(doc(db,FIREBASE_COLLECTION,snap.firestoreId),{status:"reviewed",reviewType:type,managerOrders:type==="seen"?{}:order,reviewedAt:serverTimestamp()}); alert(type==="seen"?"تم حفظ اطلاع المدير في السجل":"تم حفظ لقطة المدير في السجل"); onDone(); }catch(e){ console.error(e); alert("تعذر الحفظ"); }finally{ setBusy(false); } }
...
export default function App(){
  ...
  const pending=history.filter(s=>s.status==="submitted"||s.sentByEmployee);
  ...
  return <main className="app">
    ...
    <section className="content">
      ...
      {activeTab==="الإدارة"&&<><div className="note blue">شاشة الإدارة: خيار اطلعت يحفظ لقطة الموظف في السجل. أو أدخل طلبات المدير، أرسل للمورد لكل قسم، ثم اضغط حفظ لقطة المدير.</div>{pending.length===0?<div className="empty">لا توجد جرود بانتظار الإدارة</div>:pending.map(s=><ManagerSnapshot key={s.firestoreId||s.id} snap={s} onDone={loadHistory}/>)}</>}
